...
void main() {
  ...
  group('getNowPlayingTvSeries', () {
    ...
    test('should throw ServerException when response code is 404', () async {
      ...
      dioAdapter.onGet(
        '/tv/on_the_air',
        (request) => request.reply(404, {'status_message': 'Not Found'}),
        queryParameters: <String, dynamic>{},
      );
  ...
  group('getTvSeriesDetail', () {
    ...
    test('should throw ServerException on DioException', () async {
      ...
      when(mockDio.get<Map<String, dynamic>>(
        ...
        requestOptions: RequestOptions(path: '/tv/$tId'),
      ...
      // act & assert
      expect(
        () => testDataSource.getTvSeriesDetail(tId),
        throwsA(isA<ServerException>()),
      );
    });
  });
  ...
  group('getTvSeriesRecommendations', () {
    ...
    test('should throw ServerException on DioException', () async {
      ...
      when(mockDio.get<Map<String, dynamic>>(
        ...
        requestOptions: RequestOptions(path: '/tv/$tId/recommendations'),
        error: 'Connection error',
        type: DioExceptionType.connectionError,
      ));
      ...
      // act & assert
      expect(
        () => testDataSource.getTvSeriesRecommendations(tId),
        throwsA(isA<ServerException>()),
      );
    });
  });
  ...
  group('getSeasonDetail', () {
    ...
    test('should return season detail when response code is 200', () async {
      // arrange
      final jsonData = json.decode(readJson('dummy_data/season_detail.json'));
      dioAdapter.onGet(
        '/tv/$tTvId/season/$tSeasonNumber',
        (request) => request.reply(200, jsonData),
        queryParameters: <String, dynamic>{},
      );

      // act
      final result = await dataSource.getSeasonDetail(tTvId, tSeasonNumber);

      // assert
      expect(result, isA<SeasonDetailModel>());
    });
    ...
    test('should throw ServerException when response code is 404', () async {
      ...
      // act & assert
      expect(
        () => dataSource.getSeasonDetail(tTvId, tSeasonNumber),
        throwsA(isA<ServerException>()),
      );
    });
    ...
    test('should throw ServerException on DioException', () async {
      ...
      when(mockDio.get<Map<String, dynamic>>(
        ...
        requestOptions: RequestOptions(path: '/tv/$tTvId/season/$tSeasonNumber'),
        error: 'Connection error',
        type: DioExceptionType.connectionError,
      ));
      ...
      // act & assert
      expect(
        () => testDataSource.getSeasonDetail(tTvId, tSeasonNumber),
        throwsA(isA<ServerException>()),
      );
    });
  });
